...
const inputSchema = updateCourseInputSchema.extend({
    modules: z.array(updateModuleInputSchema.extend({
        lessons: z.array(updateLessonInputSchema.extend({
...
export class ComprehensiveUpdateCourseUsecase {
    constructor(updateCourseUsecase, updateModuleUsecase, updateLessonUsecase, updateAssignmentUsecase, uploadLessonResourceUsecase, deleteOrphanedModulesUsecase, deleteOrphanedLessonsUsecase, deleteOrphanedLessonResourcesUsecase) {
        ...
        for (const module of parsedInput.modules) {
            ...
            for (const lesson of module.lessons) {
                ...
                for (const resource of lesson.lessonResources) {
                    keptLessonResourceIds.push(resource.id);
                }
                await this.deleteOrphanedLessonResourcesUsecase.execute({ keptLessonResourceIds, lessonId: lesson.id });
            }
