...
export default class ApplyInstructorUseCase {
  ...
  async execute(applicantId, applicationData) {
    ...
    try {
      ...
      if (
        !applicationData.requestedCourseTitle ||
        applicationData.requestedCourseTitle.trim() === ""
      ) {
      ...
      if (
        !applicationData.requestedCourseSummary ||
        applicationData.requestedCourseSummary.trim() === ""
      ) {
      ...
      const newApplication = await this.applicationRepo.create({
        ...
        coverImage: applicationData.coverImage || null,
        category: applicationData.category || null,
        level: applicationData.level || null,
        durationWeeks: applicationData.durationWeeks || null,
        durationDays: applicationData.durationDays || null,
        durationHours: applicationData.durationHours || null,
        organization: applicationData.organization || null,
        language: applicationData.language || null,
        requirement: applicationData.requirement || null,
      ...
      console.error("[ApplyInstructorUseCase] Error:", error.stack || error);
      throw error;
    }
  }
}
